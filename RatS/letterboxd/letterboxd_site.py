...
class Letterboxd(Site):
    def __init__(self, args):
        ...
        self.LOGIN_USERNAME_SELECTOR = (
            login_form_selector + "//input[@id='signin-username']"
        )
        self.LOGIN_PASSWORD_SELECTOR = (
            login_form_selector + "//input[@id='signin-password']"
        )
    ...
    def _pre_login_action(self):
        time.sleep(2)
        video_elements = self.browser.find_elements_by_id(
    ...
    def _handle_cookie_notice_if_present(self):
        ...
        if cookie_notice is not None:
            cookie_accept_buttons = cookie_notice.find_elements_by_xpath(
