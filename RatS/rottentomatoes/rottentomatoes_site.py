...
class RottenTomatoes(BaseSite):
    ...
    def _pre_login_action(self):
        self.browser.find_element(By.XPATH, "//*[@id='masthead-show-login-btn']").click()
        time.sleep(1)

    def _user_is_not_logged_in(self):
        return len(self.browser.find_elements(By.CLASS_NAME, "masthead-account__user-link")) == 0

    def _get_ratings_url(self):
        time.sleep(1)  # wait for user login status to be checked
        account_link = self.browser.find_element(By.CLASS_NAME, "masthead-account__user-link").get_attribute("href")
