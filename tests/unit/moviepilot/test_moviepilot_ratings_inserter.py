...
class MoviePilotRatingsInserterTest(TestCase):
    def setUp(self):
        ...
        with open(os.path.join(TESTDATA_PATH, 'moviepilot', 'search_result.html'), encoding='UTF-8') as search_results:
            self.search_results = search_results.read()
        with open(os.path.join(TESTDATA_PATH, 'moviepilot', 'search_result_tile.html'),
    ...
    def test_insert(self, browser_mock, base_init_mock, site_mock, overview_page_mock,  # pylint: disable=too-many-arguments
                    eq_check_mock, post_rating_mock, progress_print_mock):
