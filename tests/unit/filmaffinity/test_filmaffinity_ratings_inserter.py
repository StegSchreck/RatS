...
class FilmAffinityRatingsInserterTest(TestCase):
    def setUp(self):
        ...
        with open(os.path.join(TESTDATA_PATH, 'filmaffinity', 'search_result_tile.html'),
                  encoding='UTF-8') as result_tile:
            self.search_result_tile_list = [result_tile.read()]
        with open(os.path.join(TESTDATA_PATH, 'filmaffinity', 'movie_details_page.html'),
    ...
    def test_insert(self, browser_mock, base_init_mock, site_mock, overview_page_mock,  # pylint: disable=too-many-arguments
                    eq_check_mock, search_result_page_location, progress_print_mock):
