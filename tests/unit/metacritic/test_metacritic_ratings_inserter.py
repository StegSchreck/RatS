...
class MetacriticRatingsInserterTest(TestCase):
    def setUp(self):
        ...
        with open(
            os.path.join(TESTDATA_PATH, "metacritic", "search_result_tile.html"),
            encoding="UTF-8",
        ) as result_tile:
            self.search_result_tile_list = [result_tile.read()]
        with open(
            os.path.join(TESTDATA_PATH, "metacritic", "movie_details_page.html"),
            encoding="UTF-8",
    ...
    def test_target_movie_detail_page_has_no_year(
        ...
        with open(
            os.path.join(
                TESTDATA_PATH,
                "metacritic",
                "movie_details_page_without_release_year.html",
            ),
            encoding="UTF-8",
